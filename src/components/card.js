...
const ProductCard = ({ image, brand, productName, price }) => {
  ...
  return (
    <div style={{ border: '1px solid #ccc', padding: '20px', width: '300px', margin: '10px', textAlign: 'center', boxSizing: 'border-box', borderRadius: '10px', backgroundColor: '#bbd988', borderColor: 'black' }}>
      ...
      <div style={{ margin: '10px 0' }}>
        <button onClick={() => handleQuantityChange(-1)} style={{ margin: '0 5px', padding: '5px 10px', borderRadius: '10px', fontFamily: 'Itim, cursive', fontWeight: 'bold' }}>-</button>
        <span style={{ fontFamily: 'Itim, cursive' }}>{quantity}</span>
        <button onClick={() => handleQuantityChange(1)} style={{ margin: '0 5px', padding: '5px 10px', borderRadius: '10px', fontFamily: 'Itim, cursive', fontWeight: 'bold' }}>+</button>
      </div>
      <p style={{ fontFamily: 'Itim, cursive' }}>Precio: ${price}</p>
      <button
        style={{ padding: '10px 20px', backgroundColor: '#ff7d96', cursor: 'pointer', transition: 'box-shadow 0.3s ease-in-out', borderRadius: '10px', fontFamily: 'Itim, cursive', color: '#fff' }}
        onMouseEnter={(e) => e.currentTarget.style.boxShadow = buttonHoverStyle.boxShadow}
        onMouseLeave={(e) => e.currentTarget.style.boxShadow = ''}
      >
        Agregar al Carrito
